        ...
        public virtual bool HandleEventLoopCallback()
        {
            ...
            if (SchedulingStrategy(priority: entry.Priority, scheduler: this))
            {
                try
                {
                    ...
                    AssertionsDefaultClass.FlutterError.ReportError(new FlutterErrorDetails(exception: exception, stack: exceptionStack, library: "scheduler library", context: new ErrorDescription("during a task callback"), informationCollector: (callbackStack == null) ? null : () =>
                    {
...
/// </Summary>
        ...
        private void _InvokeFrameCallback(FlutterSDK.Scheduler.Binding.FrameCallback callback, TimeSpan timeStamp, StackTrace callbackStack = default(StackTrace))
        {
            ...
            try
            {
                ...
                AssertionsDefaultClass.FlutterError.ReportError(new FlutterErrorDetails(exception: exception, stack: exceptionStack, library: "scheduler library", context: new ErrorDescription("during a scheduler callback"), informationCollector: (callbackStack == null) ? null : () =>
                {
                    yield return new DiagnosticsStackTrace("\nThis exception was thrown in the context of a scheduler callback. " + "When the scheduler callback was _registered_ (as opposed to when the " + "exception was thrown), this was the stack", callbackStack);
                }
                ));
