...
*
{
    ...
    if (parentData == null)
    {
        yield new ErrorDescription($"'{description}, which has not been set up to receive any ParentData.'");
    }
    else
    {
        yield new ErrorDescription($"'{description}, which has been set up to accept ParentData of incompatible type {parentData.GetType()}.'");
    }

    yield new ErrorHint($"'Usually, this means that the {GetType()} widget has the wrong ancestor RenderObjectWidget. '" + $"'Typically, {GetType()} widgets are placed directly inside {DebugTypicalAncestorWidgetClass} widgets.'");
    if (parentDataCreator != null)
    {
        yield new ErrorHint($"'The offending {GetType()} is currently placed inside a {parentDataCreator.GetType()} widget.'");
    }

    if (ownershipChain != null)
    {
        yield new ErrorDescription($"'The ownership chain for the RenderObject that received the incompatible parent data was:\n  {ownershipChain}'");
    }
        ...
        while (index < dirtyCount)
        {
            ...
            try
            {
                ...
                FrameworkDefaultClass._DebugReportException(new ErrorDescription("while rebuilding dirty elements"), e, stack, informationCollector: () => sync *{
                    yield new DiagnosticsDebugCreator(new DebugCreator(_DirtyElements[index]));
